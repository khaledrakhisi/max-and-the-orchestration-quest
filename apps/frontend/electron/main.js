...
// ============================================================
const PYTHON_SCRIPT_PATH_DOCKER = path.join(
  __dirname,
  "../../backend/docker_server.py",
);
const PYTHON_SCRIPT_PATH_API = path.join(
  __dirname,
  ...
  if (!unityProcess) {
    ...
    try {
      ...
      unityProcess.on("error", (err) => {
        ...
        console.error(
          "[Electron] Make sure Unity is built to: " + UNITY_EXE_PATH,
        );
        if (mainWindow && !mainWindow.isDestroyed()) {
